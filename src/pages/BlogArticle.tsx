...
const BlogArticle = () => {
  ...
  const articleJsonLd = {
    ...
    author: { '@type': 'Person', name: 'Patricia Martínez Díaz', jobTitle: 'Psicóloga Sanitaria', url: 'https://genpsicologia.com/sobre-mi' },
  ...
  const renderContent = (block: string) => {
    if (block.startsWith('### ')) return <h3 className="text-lg font-display font-semibold text-foreground mt-8 mb-3">{block.replace('### ', '')}</h3>;
    if (block.startsWith('## ')) return <h2 className="text-2xl font-display font-bold text-foreground mt-10 mb-4">{block.replace('## ', '')}</h2>;
    if (block.startsWith('- **')) {
      const match = block.match(/^- \*\*(.+?)\*\*:?\s*(.*)$/);
      if (match) return (
        <li className="flex items-start gap-2 ml-4">
          <span className="text-accent mt-1.5 shrink-0">•</span>
          <span className="text-muted-foreground leading-relaxed"><strong className="text-foreground">{match[1]}</strong>{match[2] ? `: ${match[2]}` : ''}</span>
        </li>
      );
    }
        ...
        <section className="py-12 bg-gradient-to-b from-secondary/50 to-background">
          <div className="container mx-auto px-4">
            <div className="flex items-center gap-2 mb-6 flex-wrap text-sm">
              ...
              <Link to={lp('/blog')} className="text-muted-foreground hover:text-primary transition-colors">Blog</Link>
              <span className="text-muted-foreground">/</span>
            ...
            <div className="max-w-3xl">
              ...
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold text-foreground leading-tight">{article.title}</h1>
            </div>
          </div>
        </section>
        ...
        <section className="py-16">
          <div className="container mx-auto px-4">
            <article className="max-w-3xl mx-auto space-y-4">
              {article.content.map((block, i) => <div key={i}>{renderContent(block)}</div>)}
            </article>
          </div>
        </section>
        ...
        <section className="py-12 bg-accent/5">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto bg-card rounded-2xl p-8 border border-border text-center" style={{ boxShadow: 'var(--shadow-soft)' }}>
              <h3 className="text-xl font-display font-semibold text-foreground mb-3">{s.articleCtaTitle}</h3>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
                {otherArticles.map((a) => (
                  ...
                  >
                    ...
                    <h3 className="text-base font-display font-semibold text-foreground mt-3 mb-2 group-hover:text-primary transition-colors line-clamp-2">{a.title}</h3>
